...
//
// A lightweight vanilla rewrite of the design tool's React "Tweaks" island:
// it keeps the same controls and look, but is fully self-contained — its own
// open/close button, no postMessage host protocol, and no React/Babel runtime
// (which wouldn't load offline inside a packaged Capacitor app anyway).
//
//   initTweaks({ defaults, onChange, onRestart })
//     defaults  — initial tweak values (the single source of truth lives in main.js)
//     onChange  — called with the full tweak object whenever a control changes
//     onRestart — called when "Restart run with these settings" is pressed
...
export function initTweaks({ defaults, onChange, onRestart }) {
  const t = { ...defaults };
